...
При запуске `run/plan` укажи **путь к директории кода** (целевой git-репозиторий).

План по умолчанию сохраняется в `<директория-кода>/docs`.
...
python3 openclaw.py revise-plan plan-1773055168 "добавить требования по идемпотентности и outbound retry"  # доработка в том же plan-id и исходном PR
